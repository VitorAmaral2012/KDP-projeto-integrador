...
#define PServo3 4
#define MOTOR 5
#define SensorMotor 31
#define SensorVermelho 33
#define SensorVerde 30
#define SensorAzul 32

#define BlueANGMAX 180

#define BlueANGMIN 110

#define GreenANGMIN 40
...
byte Colum[4] = {50, 48, 46, 44};
byte Linha[4] = {42, 40, 38, 36};
...
void  LigarServo(int Set, int AnguloFinal, int AnguloDeParada) {
  ...
  int i;
  Serial3.println(servos[Set].read());
...
void StartProgma() {
  ...
  if (Start_program_timer < 1500) {
    ...
    Serial3.println("Coloque os 3 kits iniciais:");
  ...
  while (Conteudo.length() != Tam) {
    ...
    if (chave == '*') {
      if (Conteudo.length() != 0) {
        ...
        Serial3.print(Componente + "=" + "\"" + Conteudo + "\"" );
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
      }
...
char pedido() {
  for (;;) {
     if(printSensor(SensorAzul, "Blue")==false){
       Serial3.print("page3.t0.txt=\"XXX\"");  //azul
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
     }
     if(printSensor(SensorVerde, "Green")==false){
       Serial3.print("page3.t1.txt=\"XXX\"");  //azul
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
      }
     if(printSensor(SensorVermelho, "Red")==false){
        Serial3.print("page3.t2.txt=\"XXX\"");  //azul
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
      }
    ...
    if (P != 'D') {
      if (P == 'A' && printSensor(SensorAzul, "Blue")) {
        Serial3.print("page4.p1.pic=6");  //azul
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
        ChangePage(4);
      ...
      if (P == 'B' && printSensor(SensorVerde, "Green")) {
        Serial3.print("page4.p1.pic=8");  //vermelho
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
        ChangePage(4);
      ...
      if (P == 'C' && printSensor(SensorVermelho, "Red")) {
        Serial3.print("page4.p1.pic=7");  //verde
        Serial3.write(0xff);
        Serial3.write(0xff);
        Serial3.write(0xff);
        ChangePage(4);
